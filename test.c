/** 
 * shell.c - Test module for the PCI simulator
 * 
 * Author: Lin Wang
...
#define CMD_WAVEFORM_WRITE          12
#define CMD_EXIT                    13
#define CMD_NUM                     14

/* Device name */
#define DEVICE_NAME "/dev/pci_simulator"
...
const char * cmdList[] = { "help", "mread", "mwrite", "read", "write", \
                           ...
                           "read32", "write32", "wfread", "wfwrite", "exit"};
...
/* Waveform write */
STATUS waveformWrite(int fd) 
{
    IO_WAVEFORM _ioWaveform;
    IO_WAVEFORM *ioWaveform = &_ioWaveform;
    ioctl(fd, WR_WAVEFORM, ioWaveform);
    return OK;
}
    ...
    for(i=0; i<sizeof(strInput); i++) {
		...
		switch (cmdIndex)
		{
			case CMD_HELP:
				...
				printf("wfread                          - read all waveforms from FPGA\n");
				printf("wfwrite                         - write all waveforms to FPGA\n");
				printf("exit                            - quit");
				break;
			...
			case CMD_WAVEFORM_READ:
	            ...
	            printf("\n");
				printf("FPGA waveform read completed.\n");
				break;
				
			case CMD_WAVEFORM_WRITE:
			    clock_gettime(CLOCK_REALTIME, &start);
				if(waveformWrite(fd) != OK)
				{
					printf("\n");
					printf("main(): waveformWrite() returns ERROR!\n");
				}
				clock_gettime(CLOCK_REALTIME, &end);
				time_spent = (end.tv_sec - start.tv_sec) * BILLION + (end.tv_nsec - start.tv_nsec);
                printf("CMD_WAVEFORM_WRITE: the elapsed time is %f nano seconds\n", time_spent);

				printf("\n");
				printf("FPGA waveform write completed.\n");
			    break;
